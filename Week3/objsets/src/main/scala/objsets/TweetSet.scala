...
abstract class TweetSet {
  ...
  def mostRetweeted: Tweet =
    mostRetweetedAcc(None).get
  
  def mostRetweetedAcc(max: Option[Tweet]) : Option[Tweet]
  /**
...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(max: Option[Tweet]) : Option[Tweet] =
  	if(max.isDefined) max else throw new java.util.NoSuchElementException
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def unionAcc(that: TweetSet, acc: TweetSet): TweetSet = {
    left.unionAcc(that, right.unionAcc(that, if (!that.contains(elem)) acc.incl(elem) else acc)) 
  }

  def descendingByRetweetAcc(acc: TweetList): TweetList = {
    val a = mostRetweeted
    remove(a).descendingByRetweetAcc(new Cons(a, acc))
  }
  
  def mostRetweetedAcc(max: Option[Tweet]) : Option[Tweet] =
    left.mostRetweetedAcc(right.mostRetweetedAcc(if(!max.isDefined || max.get.retweets > elem.retweets) Some(elem) else max))
